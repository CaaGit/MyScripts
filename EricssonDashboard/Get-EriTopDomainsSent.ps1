...
foreach ($server in $servers) {
    $logs += Get-MessageTrackingLog -ResultSize Unlimited -Start (Get-Date).AddDays(-150) -Server $server   
}
...
$statistics = foreach ($domain in $recipientDomains.Domain) {
    ...
    [PSCustomObject]@{
        Domain = $domain
        "Number of Emails" = ($recipientDomains | Where-Object { $_.Domain -eq $domain })."Number of Emails"
